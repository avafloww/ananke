//! Unified error code with structured data.
//!
...
//!
//! The OpenAPI spec uses per-endpoint error body types generated by
//! `endpoint_error!` (in `ananke-api`) to show exactly which codes
//! each endpoint can return. At runtime, all surfaces go through
//! `ApiErrorCode`'s `IntoResponse` — the per-endpoint types are
//! spec-only, not runtime types.

pub use ananke_api::shared::errors::ApiError;
use ananke_api::shared::errors::{ApiErrorCodeSlug, ApiErrorKind};
use axum::{
    Json,
    http::StatusCode,
...
mod tests {
    ...
    fn service_blocked_renders_named_peers() {
        ...
        assert_eq!(code.slug(), ApiErrorCodeSlug::ServiceBlocked);
        assert_eq!(code.status(), StatusCode::SERVICE_UNAVAILABLE);
        assert_eq!(code.kind(), ApiErrorKind::ServerError);
        assert_eq!(
            code.message(),
    ...
    fn projection_to_api_error_uses_accurate_kind() {
        ...
        assert_eq!(body.error.code, ApiErrorCodeSlug::InvalidCursor);
        assert_eq!(body.error.kind, ApiErrorKind::InvalidRequestError);
    }
}
