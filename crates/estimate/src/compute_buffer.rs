...
/// over-estimating would under-reserve and OOM them. The head-only delta
/// measured at a large ubatch sits comfortably above this `× 2` figure, which
/// confirms the direction. `n_vocab` is read from the output head's shape,
/// falling back to the token-embedding table for tied-embedding
