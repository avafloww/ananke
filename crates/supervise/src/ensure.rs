...
use ananke_allocator::placement::PackError;
...
pub(crate) enum ReservationFailure {
    ...
    /// [`ananke_estimate::EstimatorError`] so callers can tell an unknown-arch
    /// from a GGUF-read failure.
    EstimatorError(ananke_estimate::EstimatorError),
    ...
    PackFailed(ananke_allocator::placement::PackError),
}
...
pub(crate) enum MisconfiguredKind {
    ...
    InvalidTensorSplitWeights(ananke_allocator::placement::PackError),
}
...
impl std::fmt::Display for MisconfiguredKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidTensorSplitWeights(e) => {
                write!(f, "invalid tensor-split weights: {e}")
            }
        }
    }
}
...
pub(crate) fn pack_err_to_reservation_failure(
    e: ananke_allocator::placement::PackError,
) -> ReservationFailure {
    match e {
