...
impl ObservationTable {
    ...
    pub fn record_sample(&self, service: &SmolStr, vram_bytes: u64, rss: ananke_system::Rss) {
        let mut guard = self.inner.write();
        let entry = guard.entry(service.clone()).or_default();
...
/// Thin rename of [`ananke_system::ProcFs::rss`] so the snapshotter can call
...
pub fn read_rss(proc: &dyn ananke_system::ProcFs, pid: u32) -> Option<ananke_system::Rss> {
    proc.rss(pid)
}
