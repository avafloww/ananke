//! Generic per-service cache of model metadata + estimator output.
//!
//! The daemon instantiates `EstimateCache<ananke_estimate::Estimate>` so
//! the detail handler can re-run the packer for the placement preview
//! without re-parsing the GGUF. The cache itself is generic over the
//! full-estimate payload because `ananke-estimate` must not be a
//! dependency of this leaf crate.
...
/// Cloneable handle to a daemon-wide estimate cache. Backed by a
...
pub struct EstimateCache<T> {
    inner: Arc<RwLock<BTreeMap<SmolStr, CacheEntry<T>>>>,
}

impl<T> Default for EstimateCache<T> {
    fn default() -> Self {
        Self {
...
impl<T> EstimateCache<T> {
    ...
    pub fn insert(&self, name: SmolStr, entry: CacheEntry<T>) {
...
mod tests {
    ...
    fn make_entry(model: &str, mmproj: Option<&str>, fp: u64) -> CacheEntry<u64> {
        CacheEntry {
    ...
    /// Pin the fingerprint-based invalidation: paths unchanged
