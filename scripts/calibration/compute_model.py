...
# Fixture inputs spanning every column: a narrow and a wide model, both batch
# sizes either side of the attention chunk, one and four streams, an f16 and a
# quantised cache, head and non-head cards, replicated and unreplicated masks,
# and expert offload on and off.
FIXTURE_CASES = (
    dict(ubatch=512, n_kv=32768, ctx=32768, quantised=False, head_share=1.0,
         n_vocab=151936, n_embd=2048, offloaded=0, mask_copies=1),
    dict(ubatch=512, n_kv=32768, ctx=32768, quantised=False, head_share=0.0,
         n_vocab=151936, n_embd=2048, offloaded=0, mask_copies=4),
    dict(ubatch=2048, n_kv=8192, ctx=32768, quantised=True, head_share=0.5,
         n_vocab=262144, n_embd=5376, offloaded=40, mask_copies=1),
    dict(ubatch=256, n_kv=131072, ctx=131072, quantised=True, head_share=1.0,
         n_vocab=100352, n_embd=3072, offloaded=30, mask_copies=4),
    dict(ubatch=1024, n_kv=65536, ctx=262144, quantised=False, head_share=0.25,
         n_vocab=32000, n_embd=4096, offloaded=1, mask_copies=1),
)


def fixture_document() -> dict:
    """The design columns for a fixed set of inputs, for the Rust side to match.

    `build.rs` already asserts that the column *names* in `tuning.json` match the
    generated struct, which catches a column added on one side and not the other.
    It cannot catch the two sides computing a column *differently* — an off-by-one
    in a stream divisor, or one side forgetting that the mask carries its
    replication count. This pins the values.

    Regenerated by `analyse.py emit`, so changing the Python here fails the Rust
    test until the evaluator is changed to match, which is the whole point.
    """
    return {
        "$comment": (
            "Generated by scripts/calibration/analyse.py emit. The design columns "
            "of the compute model for fixed inputs, asserted against "
            "estimator::compute_model::Columns so the fitter and the evaluator "
            "cannot compute the same column differently."),
        "columns": list(COLUMNS),
        "cases": [
            {"inputs": case, "columns": columns(**case)}
